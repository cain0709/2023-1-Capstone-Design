...
class utils:
	...
	def make_query_ML_classificaiton(self, data_path, selected_model,  label_col, class_cnt, nan_check_flag, useless_features, numeric_col, categorical_col,
		...
		Query += f'Do not use pruner and use the TPESampler, please add library "from optuna.samplers import TPESampler".\n'
		Query += f'Continue with these steps, set the evaluation metric inside the objective function to macro f1-score.\n'
	...
	def make_query_ML_regression(self, data_path, selected_model, label_col, nan_check_flag, useless_features, numeric_col, categorical_col, scaler_flag):
		...
		Query += f'Please import LabelEncoder, train_test_split, optuna, TPESampler, mean_squared_error on the code.\n'
		...
		Query += f'Also, separate 10 percent of the validation data using the train_test_split function, and do not use stratify=y(the stratify parameter of the function).\n' 
		Query += f'In these code, set all SEED values (and random_state parameters) to 42. \n'
		...
		Query += f'After these steps, print best hyperparameters and score and valid mse.\n'
